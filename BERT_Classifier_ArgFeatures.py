...
# Extract Argumentation features
arg_features = ['User1_Quality', 'User1_Argumentativeness', 'User1_Sentiment', 'User2_Quality', 'User2_Argumentativeness', 'User2_Sentiment']

# Create 
train_dataset = CustomDataset(
    train_df['User1_Embedding'].tolist(),
    train_df['User2_Embedding'].tolist(),
    train_df[arg_features].values.tolist(),
    train_df['relationship'].tolist()
)
test_dataset = CustomDataset(
    test_df['User1_Embedding'].tolist(),
    test_df['User2_Embedding'].tolist(),
    test_df[arg_features].values.tolist(),
...
model.save_pretrained('./BERT_ArgFeatures_trained_model')
